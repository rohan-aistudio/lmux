...
def load_registry() -> dict:
    ...
    return {"models": {}, "next_port": MODEL_PORT_START, "platform": {}, "models_path": ""}
...
def write_compose(plat: dict, cpu_override: bool = False, models_path: str = ""):
...
"""
    ...
    # Use custom models path or default ./models
    models_volume = models_path if models_path else "./models"
...
def download_gguf(repo_id: str, filename: str, token: str, models_dir: Path | None = None) -> Path:
    try:
...
def cmd_init(args):
    ...
    models_path  = getattr(args, "models_path", "") or ""
    ...
    if models_path:
        ...
        ok(f"Models path: {models_path}")
    else:
        MODELS_DIR.mkdir(parents=True, exist_ok=True)
        ok("models/ directory ready")

    # Write files
    write_compose(plat, cpu_override, models_path)
    ok("docker-compose.yml generated")

    reg = load_registry()
    reg["platform"] = plat
    if models_path:
        reg["models_path"] = models_path
    save_registry(reg)
    ...
    if models_path:
        print(f"    Models →  {CYAN}{models_path}{RESET}")
    print()
...
def cmd_pull(args):
    ...
    # Download — use custom models path if configured
    custom_path = reg.get("models_path", "")
    models_dir = Path(custom_path) if custom_path else None
    local_path = download_gguf(repo_id, filename, token, models_dir)
...
def _install_aliases():
    ...
    if system in ("Linux", "Darwin"):
        ...
        alias_line = f"\nalias lmux='{python_bin} {lmux_path}'\n"
        ...
        ps_line = f'\nfunction lmux {{ & "{python_bin}" "{lmux_path}" @args }}\n'
        try:
            ...
            info(f"Add manually: function lmux {{ & \"{python_bin}\" \"{lmux_path}\" @args }}")
...
def main():
    ...
    p_init.add_argument("--models-path", help="Custom path for model storage (e.g. /mnt/nvme/models)")
